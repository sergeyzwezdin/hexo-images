...
const cleanupManifestFromRemovedFiles = (hexo, imageManifestPath, checkOnly) => {
    ...
    if (fs.existsSync(imageManifestPath)) {
        ...
        const manifestFolderPath = path.dirname(imageManifestPath);
        ...
        for (const imagePath of Object.keys(manifest)) {
            ...
            const isGeneratedFilesMissing = [
                ...new Set(
                    Object.keys(manifest[imagePath].files)
                        .map((f) => {
                            if (typeof manifest[imagePath].files[f] === 'string') {
                                return manifest[imagePath].files[f];
                            } else if (typeof manifest[imagePath].files[f] === 'object') {
                                return Object.keys(manifest[imagePath].files[f]).map((ff) => manifest[imagePath].files[f][ff]);
                            }
                        })
                        .flat(Infinity)
                )
            ]
                .map((f) => path.join(manifestFolderPath, f))
                .some((f) => !fs.existsSync(f));

            if (!isImageExists || isGeneratedFilesMissing) {
                if (!checkOnly) {
